use leptos::prelude::*;
use lucide_leptos::{
    ArrowRight, CircleCheck, DatabaseZap, FileSearch, Hash, Info, ScanLine, Upload,
};

#[component]
pub fn VerifyStepsFlow(on_show_more: Callback<()>) -> impl IntoView {
    view! {
        <div class="flex flex-col gap-2">
            <div class="flex items-center gap-1.5 flex-wrap">
                <Step icon_slot=view! { <Upload size=13 color="#d20f39" /> }.into_any()       label="Upload" />
                <ArrowRight size=11 color="#d1d5db" />
                <Step icon_slot=view! { <FileSearch size=13 color="#d20f39" /> }.into_any()   label="Extract" />
                <ArrowRight size=11 color="#d1d5db" />
                ...
                <ArrowRight size=11 color="#d1d5db" />
                <Step icon_slot=view! { <CircleCheck size=13 color="#16a34a" /> }.into_any()  label="Verdict" />
